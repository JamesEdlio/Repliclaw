...
async function main() {
  ...
  const markerLine = `${MARKER_TAG} ${MARKER_EVENT} run_id=${ctx.runId} ts=${sentAt} skill_version=${SKILL_VERSION}${triggeredBy ? ` triggered_by=${triggeredBy}` : ""}${gmailThreadId ? ` gmail_thread_id=${gmailThreadId}` : ""}${gmailMessageId ? ` gmail_message_id=${gmailMessageId}` : ""}${gmailThreadId || gmailMessageId ? ` gmail_mailbox=${GMAIL_MAILBOX}` : ""}${gmailRfc822Id ? ` rfc822_message_id=${gmailRfc822Id}` : ""}`;
